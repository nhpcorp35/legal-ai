...
def build_request(case_id: str, pages: list[dict[str, Any]], correction: str = "") -> dict[str, Any]:
    ...
    instructions = "Use only these verified pages. Do not infer missing facts. Every finding needs one or more literal, at-least-12-character quotes. If a category is unsupported, put it in unresolved_questions. Alternative pleading concerns different possible factual bases that may be true but are not yet known with specificity. Never label pleaded positions, or different requests for relief, inconsistent or contradictory solely because they differ. Treat indemnity, contribution, and partial indemnity as requests that may be alternative, cumulative, or fallback relief; do not decide that issue from excerpts. Attribute each relief position to the pleading party: say that the answering third-party defendant alleges, requests, or claims it, rather than stating it as Homie's conclusion. When the text says a party is entitled to relief, preserve that attribution and identify the underlying factual and pleading context as unresolved. When a verified excerpt expressly identifies a cross-claim target or the party from whom relief is requested, state that target directly; do not call it unresolved merely because the complete pleading or factual basis is absent. In a cross-claim, separately track the target named in the claim heading or operative request and the party named in any following relief paragraph; support each with its own literal quote. If the heading or operative request names one party and the following relief paragraph names another, report both express statements separately. Do not say that either named party, the intended target, or whether relief is sought from one, the other, or both cannot be resolved; only the relationship between the two pleaded statements or their omitted factual context may remain unresolved."
    if correction:
