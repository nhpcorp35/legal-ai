...
STRATEGIC_PROCEDURAL_RECORD_RE = re.compile(
    r"(?:(?:^|[^A-Za-z])order[\s_]+to[\s_]+show[\s_]+cause|temporary\s+restraining\s+order|"
...
def finding_schema(page_citation_properties, *, attorney_sections=False, litigation_map=False, strategic_analysis=False, reasoning_mode="strategic_analysis", max_findings=8, statement_max_length=900, fixed_motion_record_support=False):
...
def generate(question, pages, coverage=None, authorities=None):
    ...
    direct_agencies = {
        issuer for page in pages if (issuer := direct_agency_issuer(page)) is not None
    }
    # A motion recommendation has five required non-record sections.  Where
    # the selected record contains two direct agencies, each must receive its
    # own Record support finding alongside expert and measurement evidence.
    # Nine findings is therefore the smallest schema budget that lets the
    # model satisfy the fail-closed record-coverage validator.
    max_findings = (
        9
        if strategic_question
        and reasoning_mode == "motion_recommendation"
        and len(direct_agencies) > 1
        else len(LITIGATION_MAP_SECTIONS) if map_question else 8
    )
    fixed_motion_record_support = (
        strategic_question
        and reasoning_mode == "motion_recommendation"
        and len(direct_agencies) > 1
    )
    operative_tro_selected = any(
        OPERATIVE_TRO_TEXT_RE.search(str(page.get("text", "")))
        for page in pages
    )
    schema=finding_schema({"source_sha256":{"type":"string"},"filename":{"type":"string"},"page_number":{"type":"integer","minimum":1}}, attorney_sections=bool(authorities) and not strategic_question, litigation_map=map_question, strategic_analysis=strategic_question, reasoning_mode=reasoning_mode, max_findings=max_findings, statement_max_length=2400 if third_party_action_count > 1 else 1200 if strategic_question else 900, fixed_motion_record_support=fixed_motion_record_support)
    ...
    if TOP_ATTACK_SURFACES_MARKER in question.casefold():
        ...
        if reasoning_mode == "motion_recommendation":
            instructions += " The attorney asks which motions to consider. Use every section in this exact order: Objective and posture; Candidate motions; Record support; Likely opposition; Gaps and prerequisites; Recommendation. Identify only motions supported by the verified posture and record. For each candidate, state the target, required showing only when verified authority supplies it, record support, strongest opposition, prerequisite proof or procedural step, and comparative reason to prioritize or reject it. When the supplied record includes expert opinion, a direct regulatory record, or drawing/measurement evidence, return three separate Record support findings before Likely opposition: one citing expert evidence, one citing the direct regulatory record, and one citing visual or measurement evidence. Do not combine or omit those categories, and do not put their analysis only in another section. When the supplied direct regulatory records identify more than one agency, cite and analyze each agency's record; an expert's account or a subpoena request cannot substitute for the direct record."
            if operative_tro_selected:
                instructions += " The selected verified record contains operative signed TRO terms. State those cited terms and do not call the order, its terms, or its duration missing, illegible, or unresolved. You may identify only later modification, extension, service, or present-status evidence as unresolved when the cited pages do not establish it."
            else:
                instructions += " If the exact signed TRO terms, duration, or current status are unresolved, do not recommend filing a TRO-modification motion now; instead identify the order as a prerequisite and limit any recommendation to preserving rights, obtaining the order, or seeking a record-supported conference or hearing."
            ...
            if len(direct_agencies) > 1:
                agency_names = ", ".join(sorted(direct_agencies))
                instructions += f" This record has separate mandatory direct-agency sources: {agency_names}. The JSON schema fixes the nine findings in this order: Objective and posture; Candidate motions; Record support for expert evidence; Record support for the nysdec record; Record support for the usace record; Record support for visual or measurement evidence; Likely opposition; Gaps and prerequisites; Recommendation. Each agency finding must cite its own direct record; do not combine the agencies into one finding."
