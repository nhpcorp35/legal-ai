...
def search_indexed_case(case_id, query):
    """Search one protected verified record and return source excerpts only."""
    gateway_url = os.environ.get("LEGALAI_REVIEW_GATEWAY_URL", "").rstrip("/")
    secret = os.environ.get("LEGALAI_REVIEW_GATEWAY_SECRET", "")
    if not gateway_url or not secret:
        return None
    ...
    request_data = urllib.request.Request(
        f"{gateway_url}/portal/cases/{urllib.parse.quote(case_id, safe='')}/search",
        data=payload,
        headers={
            "Content-Type": "application/json",
            "X-LegalAI-Portal-Secret": secret,
...
def run_framework_evidence_check(case_id):
    ...
    if not isinstance(conflicts, list) or conflict_map.get("model_called") is not False:
        return None
    authority_verification = result.get("authority_verification")
    if not isinstance(authority_verification, dict) or authority_verification.get("model_called") is not False:
...
TOP_ATTACK_SURFACES_QUESTION = (
    ...
    "(1) contradictions or tensions between identified filings or evidence; "
    "(2) credibility vulnerabilities grounded in a specific record conflict, omission, or inconsistency; and "
    "(3) procedural weaknesses tied to the pleaded posture, burden, remedy, notice, timing, or preservation shown in the record. "
    "For each ranked item, identify the category, the precise record-supported vulnerability, the affected party or position only when expressly shown, "
    "and the verified pages supporting it. Do not treat alternative pleading, denial, or advocacy as an established contradiction; identify missing information where the record does not support a conclusion."
)
...
def load_draft_requests(case_id, *, force_refresh=False):
    ...
    if not gateway_url or not secret:
        return None
    request_data = urllib.request.Request(
        f"{gateway_url}/portal/cases/{urllib.parse.quote(case_id, safe='')}/draft-requests",
        headers={"X-LegalAI-Portal-Secret": secret},
        method="GET",
    )
    try:
...
def _direct_b2_draft_record(s3, bucket, case_id, request_id):
    ...
    replacement_of = request_item.get("regenerate_from_request_id")
    if replacement_of is not None:
        if not re.fullmatch(r"draft-[0-9]+-[0-9a-f]{12}", str(replacement_of)):
            return None
        record["regenerate_from_request_id"] = replacement_of
    if status == "READY":
...
# Direct canonical verified-page search; the gateway remains an outage fallback.
_gateway_search_indexed_case = search_indexed_case


def _direct_b2_search_indexed_case(case_id, query):
    s3, bucket = _operator_regeneration_b2_client()
    if not s3 or not bucket:
        return None
    terms = re.findall(r"[a-z0-9]{2,}", normalize_for_search(query))[:8]
    if not terms:
        return []
    identity = _direct_b2_json(s3, bucket, f"cases/{case_id}/intake/case_identity.json")
    original = identity.get("source_sha256") if isinstance(identity, dict) else None
    ...
    sources = [original]
    if isinstance(source_set, dict) and source_set.get("case_id") == case_id:
        candidate = source_set.get("sources")
        digests = [item.get("source_sha256") for item in candidate if isinstance(item, dict)] if isinstance(candidate, list) else []
        if digests and all(isinstance(digest, str) and re.fullmatch(r"[0-9a-f]{64}", digest) for digest in digests) and len(set(digests)) == len(digests) and original in digests:
            sources = digests
    ...
    for source_sha256 in sources:
        try:
            raw = s3.get_object(
                Bucket=bucket,
                Key=f"cases/{case_id}/intake/source/{source_sha256}/page_records.jsonl",
            )["Body"].read()
            rows = raw.decode("utf-8").splitlines()
        except Exception:
            return []
        for row in rows:
            ...
            if not (
                isinstance(filename, str)
                and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._ -]{0,180}\.pdf", filename)
                and isinstance(page_number, int) and 1 <= page_number <= 100000
            ...
            if matched != len(terms):
                continue
            first = min(normalized.find(term) for term in terms)
            start = max(0, first - 300)
            snippet = " ".join(text[start:start + 1200].split())
            results.append((matched, filename.casefold(), page_number, {
                "source_sha256": source_sha256,
                "filename": filename,
                "page_number": page_number,
                "snippet": snippet,
            }))
    return [item[3] for item in sorted(results, key=lambda item: (-item[0], item[1], item[2]))[:25]]


def search_indexed_case(case_id, query):
    """Search canonical verified page records directly from B2 before gateway."""
    direct = _direct_b2_search_indexed_case(case_id, query)
    return _gateway_search_indexed_case(case_id, query) if direct is None else direct
...
def load_draft_review_feedbacks(reviewer, case_id, request_ids):
    """Load the current reviewer's exact feedback for a bounded draft set."""
...
def rennick_evaluation_summary(reviewer):
        ...
        if isinstance(dimensions, dict):
            for key in dimension_scores:
                ...
                if isinstance(score, int) and 1 <= score <= 5:
                    dimension_scores[key].append(score)
        questions.append(
            {
                "question": item["question"],
                "request_id": request_id,
                ...
                "accuracy": record.get("accuracy_rating") if record else None,
                "usefulness": record.get("usefulness_rating") if record else None,
                "awaiting_paid_run": request_id is None,
            }
        )
    dimensions = []
    for key, label, question in _DRAFT_REVIEW_DIMENSIONS:
    ...
    return {
        ...
        "reviewable_count": len(request_ids),
    }
...
def findings_with_verified_authorities(findings):
    """Attach registry records to known authority ids and ignore all others."""
    rendered = []
    for finding in findings if isinstance(findings, list) else []:
        ...
        rendered.append({
            ...
            "authorities": [
                ...
                and authority_id in _VERIFIED_AUTHORITY_BY_ID
            ],
        })
    return rendered


def group_attorney_findings(findings):
    """Group newly structured findings while keeping legacy drafts readable."""
    groups = []
    for finding in findings:
        section = clean_text(finding.get("section", ""))
        if not groups or groups[-1]["name"] != section:
            groups.append({"name": section, "findings": []})
...
def discard_temporary_draft_request(case_id, request_id):
    ...
    if not re.fullmatch(r"draft-[0-9]+-[0-9a-f]{12}", str(request_id or "")):
        return False
    request_data = urllib.request.Request(
        f"{gateway_url}/portal/cases/{urllib.parse.quote(case_id, safe='')}/draft-requests/{request_id}/discard-test",
        headers={"X-LegalAI-Portal-Secret": secret},
        method="POST",
    )
    try:
...
def _direct_b2_create_draft_request(case_id, question, reviewer, regenerate_from=None):
    """Write an auditable request directly to B2, including safe regeneration."""
    s3, bucket = _operator_regeneration_b2_client()
    if not s3 or not bucket:
        return None
    existing = _direct_b2_draft_records(case_id) or []
    if regenerate_from is not None:
        if not re.fullmatch(r"draft-[0-9]+-[0-9a-f]{12}", str(regenerate_from)):
            return {"ok": False, "blocked": "invalid_original_request"}
        original = next(
            (item for item in existing if item.get("request_id") == regenerate_from),
            None,
        )
        if (
            original is None
            or original.get("status") != "READY"
            or original.get("requested_by") != reviewer
        ):
            return {"ok": False, "blocked": "regeneration_not_permitted"}
        replacement = next(
            (
                item for item in existing
                if item.get("regenerate_from_request_id") == regenerate_from
                and item.get("requested_by") == reviewer
            ),
            None,
        )
        if replacement is not None:
            return {"ok": True, "request_id": replacement["request_id"], "reused": True}
        question = original["question"]
    else:
        for item in existing:
            if (
                item.get("question", "").strip().casefold() == question.strip().casefold()
                and item.get("status") in {"QUEUED", "RUNNING", "READY"}
            ):
    ...
    if regenerate_from is not None:
        request_record["regenerate_from_request_id"] = regenerate_from
    status_record = {
...
def create_draft_request(case_id, question, reviewer, regenerate_from=None):
    direct = _direct_b2_create_draft_request(
        case_id, question, reviewer, regenerate_from=regenerate_from
    )
    if direct is not None:
        # A direct authorization failure must never fall through to the gateway.
        return direct if direct.get("ok") else None
...
def open_szymczyk_verified_pdf(document_name):
    """Retrieve one verified Szymczyk source PDF through the protected relay."""
    document_name = clean_text(document_name)
    gateway_url = os.environ.get("LEGALAI_REVIEW_GATEWAY_URL", "").rstrip("/")
    secret = os.environ.get("LEGALAI_REVIEW_GATEWAY_SECRET", "")
    if not gateway_url or not secret or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._ -]{0,180}\.pdf", document_name):
        return None
    request_data = urllib.request.Request(
        f"{gateway_url}/portal/szymczyk/verified-pdf",
        data=json.dumps({"document_name": document_name}).encode("utf-8"),
        headers={
            "Content-Type": "application/json",
            "X-LegalAI-Portal-Secret": secret,
        },
    ...
    try:
        ...
        return None
    if content_type != "application/pdf" or not pdf:
        return None
    return pdf
...
def visible_answered_drafts(draft_requests):
    """Return the current completed answers shown to an attorney.

    Immutable superseded drafts remain in storage and in their audit trail, but
    they must not inflate the workspace's attorney-facing answer count.
    """
    answered = [
        item for item in (draft_requests or [])
...
def attorney_workspace():
    ...
    if case00_questions:
        matters.append(
            {
                "name": "Case-00 Triborough",
                "description": "Prepared attorney-review questions with source-supported candidate answers.",
    ...
    for case in registered_cases:
        if case["case_id"] not in known_case_ids:
            ...
            if case["stage"] == "Verified source indexed":
                ...
                answered_count = len(visible_answered_drafts(draft_requests))
...
def workspace_indexed_case_search(case_id):
    ...
    if request.method == "POST":
        query = clean_text(request.form.get("query", ""))
        if not query or len(query) > 500:
            error = "Enter a search query of up to 500 characters."
        else:
            results = search_indexed_case(case_id, query)
            if results is None:
...
def workspace_framework_evidence(case_id):
    ...
    if result and isinstance(result.get("authority_verification"), dict):
        authority = result["authority_verification"]
        candidates = authority.get("candidates")
        if isinstance(candidates, list) and len(candidates) > 10:
            result["authority_verification"] = {
                **authority,
                "candidates": candidates[:10],
                "analysis_guardrail": (
                    f"Showing the first 10 of {len(candidates)} unique party-cited "
                    "authorities. They remain unverified until matched to an official "
                    "primary source; the complete audit remains available to the system."
                ),
            }
    unavailable = request.method == "POST" and result is None
    return render_template_string(
        """<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Framework evidence check</title><style>:root{font-family:Georgia,serif;color:#172331;background:#f6f8fb}body{margin:0}main{max-width:940px;margin:0 auto;padding:42px 24px 64px}a{color:#123f63}p{font-size:1.05rem;line-height:1.55}.panel,.result{background:#fff;border:1px solid #cbd5e1;border-radius:10px;padding:22px;margin-top:20px}button{background:#123f63;color:#fff;border:0;border-radius:6px;padding:11px 15px;font:inherit;font-weight:bold;cursor:pointer}.meta{color:#52606d}.notice{border-left:4px solid #b45309;padding:12px 14px;background:#fffbeb}.snippet{white-space:pre-wrap;overflow-wrap:anywhere}.lane{border-left:4px solid #245b83;padding-left:16px}</style></head><body><main><p><a href="/workspace">← Attorney workspace</a></p><h1>Framework evidence check</h1><p class="meta">{{ case_id }}</p><p>Reads the immutable verified record only. It does not call a model, create a draft, or change B2.</p><section class="panel"><form method="post"><button type="submit">Run evidence check</button></form></section>{% if unavailable %}<p class="notice">The check is temporarily unavailable. No source or draft was changed.</p>{% endif %}{% if result is not none %}<p><strong>Completed with no model call.</strong></p>{% if result.authority_verification %}<h2>Authority verification status</h2><p class="meta">Party-filed citations are not treated as governing law until independently verified from a primary source.</p><p>{{ result.authority_verification.analysis_guardrail }}</p>{% for candidate in result.authority_verification.candidates %}<section class="panel lane"><h3>{{ candidate.citation }}</h3><p><strong>Status:</strong> {{ candidate.status.replace('_', ' ') }}</p><p>{{ candidate.verification_requirement }}</p>{% if candidate.primary_source %}<p><strong>Official primary source:</strong> <a href="{{ candidate.primary_source.source_url }}" target="_blank" rel="noopener">{{ candidate.primary_source.title }}</a><br><span class="meta">{{ candidate.primary_source.issuing_body }} · reporter page {{ candidate.primary_source.reporter_page }}</span></p>{% endif %}<p class="meta">Record citations: {% for cite in candidate.record_citations %}{{ cite.filename }} p. {{ cite.page_number }}{% if not loop.last %}; {% endif %}{% endfor %}</p></section>{% else %}<p class="notice">No recognizable case, statute, or rule citation was found in the verified record.</p>{% endfor %}{% endif %}<h2>Framework conflict map</h2><p class="meta">Candidate tensions only—not findings of fact, liability, or governing law.</p>{% for lane in result.conflicts %}<section class="panel lane"><h3>{{ lane.name.replace('_', ' ').title() }}</h3><p>{{ lane.guardrail }}</p><p><strong>Status:</strong> {{ lane.status.replace('_', ' ') }}</p><h4>{{ lane.left.record_role.replace('_', ' ').title() }}</h4>{% for entry in lane.left.results %}<article class="result"><p><strong>{{ entry.filename }}</strong> — PDF page {{ entry.page_number }}</p><p class="snippet">{{ entry.snippet }}</p></article>{% endfor %}<h4>{{ lane.right.record_role.replace('_', ' ').title() }}</h4>{% for entry in lane.right.results %}<article class="result"><p><strong>{{ entry.filename }}</strong> — PDF page {{ entry.page_number }}</p><p class="snippet">{{ entry.snippet }}</p></article>{% endfor %}</section>{% endfor %}<h2>Evidence lanes</h2>{% for label, entries in result.categories.items() %}<h3>{{ label.replace('_', ' ').title() }}</h3>{% for entry in entries %}<article class="result"><p><strong>{{ entry.filename }}</strong> — PDF page {{ entry.page_number }}</p><p class="snippet">{{ entry.snippet }}</p></article>{% endfor %}{% else %}<p class="notice">No matching verified pages were found.</p>{% endfor %}{% endif %}</main></body></html>""",
    ...
    if request.method == "POST":
        ...
        if action == "preview":
            ...
            if confirmation is None:
                error = "The v4.0 report request could not be saved. Please try again."
            else:
                return redirect(
                    url_for(
            ...
            elif request.form.get("paid_regeneration_confirmation") != "approve_paid_internal_replacement":
                error = "Confirm the paid internal replacement run before regenerating."
            elif prior.get("requested_by") != reviewer:
                error = "Only the attorney who requested this draft can regenerate it."
            else:
...
@app.route("/workspace/matters/<path:case_id>/drafts/<request_id>/status")
@app.route("/workspace/case-00/drafts/<request_id>/status", defaults={"case_id": CASE00_ID})
def workspace_matter_draft_status(case_id, request_id):
    """Return a minimal authenticated status for a page already awaiting one draft."""
    if basic_review_user() is None:
        return basic_auth_required_response()
    if not re.fullmatch(r"draft-[0-9]+-[0-9a-f]{12}", request_id):
        abort(404)
    ...
    if status == "FAILED":
        raw_code = item.get("failure_code")
        payload["failure_code"] = (
            raw_code
            if isinstance(raw_code, str) and re.fullmatch(r"[a-z_]{1,40}", raw_code)
            else None
        )
    return Response(
        json.dumps(payload),
...
def workspace_matter_drafts(case_id):
    ...
    if case_id != CASE00_ID:
        ...
        if registered.get(case_id) != "Verified source indexed":
            abort(404)
    answered = visible_answered_drafts(load_draft_requests(case_id))
    return render_template_string(
...
@app.route("/workspace/matters/<path:case_id>/review-packet/evaluation")
def workspace_rennick_attorney_evaluation(case_id):
    """Show a read-only, no-model rollup of the current attorney's reviews."""
    reviewer = basic_review_user()
    if reviewer is None:
        return basic_auth_required_response()
    if case_id != RENNICK_FRAMEWORK_CASE_ID:
        abort(404)
    summary = rennick_evaluation_summary(reviewer)
    return render_template_string(
        """<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Rennick Evaluation Summary</title><style>:root{font-family:Georgia,serif;color:#172331;background:#f6f8fb}body{margin:0}main{max-width:900px;margin:0 auto;padding:42px 24px 64px}a{color:#123f63}h1{margin:0 0 8px;font-size:clamp(2rem,5vw,3rem)}h2{margin:28px 0 10px;color:#123f63}p,li,td,th{font-size:1.05rem;line-height:1.5}.meta{color:#52606d}.panel{background:#fff;border:1px solid #cbd5e1;border-radius:10px;padding:22px;margin-top:24px;box-shadow:0 2px 8px #0f172a10}.notice{border-left:5px solid #123f63;background:#eef6fc}table{border-collapse:collapse;width:100%;margin-top:12px}th,td{text-align:left;vertical-align:top;border-bottom:1px solid #cbd5e1;padding:10px 8px}th{color:#123f63}.pending{color:#7c2d12;font-weight:bold}.reviewed{color:#166534;font-weight:bold}</style></head><body><main><p><a href="{{ url_for('workspace_rennick_attorney_review_packet', case_id=case_id) }}">← Rennick attorney review packet</a></p><h1>Rennick Evaluation Summary</h1><p class="meta">This is a read-only rollup of submitted attorney scores. It does not generate or revise a draft.</p><section class="panel notice">{% if summary.reviewed_count %}<p><strong>{{ summary.reviewed_count }} of {{ summary.reviewable_count }} available analyses reviewed.</strong></p>{% else %}<p><strong>No attorney evaluation has been submitted yet.</strong> The score table will populate after a review is saved in the packet.</p>{% endif %}</section><section class="panel"><h2>Question coverage</h2><table><thead><tr><th>Question</th><th>Review status</th><th>Result</th></tr></thead><tbody>{% for item in summary.questions %}<tr><td>{{ item.question }}</td><td>{% if item.awaiting_paid_run %}<span class="pending">Not generated</span>{% elif item.reviewed %}<span class="reviewed">Reviewed</span>{% else %}<span class="pending">Not yet reviewed</span>{% endif %}</td><td>{% if item.awaiting_paid_run %}Separate paid-run approval required.{% elif item.reviewed %}{{ item.decision|replace('_', ' ')|title }} · Accuracy {{ item.accuracy }}/5 · Usefulness {{ item.usefulness }}/5{% else %}—{% endif %}</td></tr>{% endfor %}</tbody></table></section><section class="panel"><h2>Attorney-quality scores</h2><table><thead><tr><th>Dimension</th><th>Average</th><th>Meaning</th></tr></thead><tbody>{% for dimension in summary.dimensions %}<tr><td>{{ dimension.label }}</td><td>{% if dimension.average is not none %}{{ dimension.average }}/5 ({{ dimension.review_count }} review{{ '' if dimension.review_count == 1 else 's' }}){% else %}—{% endif %}</td><td>{{ dimension.question }}</td></tr>{% endfor %}</tbody></table></section></main></body></html>""",
        case_id=case_id,
        summary=summary,
    )
...
def workspace_szymczyk_pdf(filename):
    """Serve one cited verified PDF after the existing workspace authentication."""
    if basic_review_user() is None:
        return basic_auth_required_response()
    document_name = clean_text(filename)
    pdf = open_szymczyk_verified_pdf(document_name)
    if pdf is None:
        abort(404)
    return send_file(
        BytesIO(pdf),
